const contractAddress = "0xE0C69D5742C4Ddc127E6a0c830D81B7b006dEcD0"; // contract address
const contractABI = [
	{
		"inputs": [
			{
...
async function initialize() {
  if (window.ethereum) {
    ...
    const role = account.toLowerCase() === adminAddress.toLowerCase() ? "admin" : "user";
...
function listenForAccountChange() {
  window.ethereum.on("accountsChanged", (accounts) => {
    ...
    const adminAddress = contract.methods.admin().call();
    const role = account.toLowerCase() === adminAddress.toLowerCase() ? "admin" : "user";
    document.getElementById("role").innerText = `Role: ${role}`;
  });
}
...
async function rateCandidate(candidateId) {
  ...
  if (isNaN(rating) || rating < 1 || rating > 5) {
    ...
    // Check if the user has already rated this candidate
    const hasRated = await contract.methods.hasRated(account, candidateId).call();
    if (hasRated) {
      alert("You have already rated this candidate!");
      return;
    }

    // Proceed to rate the candidate
