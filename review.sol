...
contract ReviewSystem {
    ...
    mapping(address => mapping(uint => bool)) public hasRated; // Mapping to track if an address has rated a candidate
    ...
    function rate(uint _candidateId, uint _rating) public {
        require(!hasRated[msg.sender][_candidateId], "You have already rated this candidate!");
        require(_candidateId > 0 && _candidateId <= candidatesCount, "Invalid candidate ID");
        require(_rating >= 1 && _rating <= 5, "Rating must be between 1 and 5");

        hasRated[msg.sender][_candidateId] = true; // Mark the candidate as rated by the user
